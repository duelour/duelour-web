...
export const listPlayerChallengesByStatusOnce = (displayName, status = 'active') => {
  if (!displayName) {
    throw new Error('displayName not provided!');
  }

  // TODO: paginate this!
  return playersDbByCustom(`/${displayName.toLowerCase()}/challenges/${status}`).orderByPriority().once('value');
};
