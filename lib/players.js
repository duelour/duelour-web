...
export const listPlayerChallenges = (displayName, cb, opts = {}) => {
  if (!displayName) {
    throw new Error('displayName not provided!');
  }
  ...
  // TODO: paginate this!
  if (opts.detatch) {
    return playersDbByCustom(`/${displayName}/challenges`).off('value', cb);
  }
  return playersDbByCustom(`/${displayName}/challenges`).on('value', cb);
};
