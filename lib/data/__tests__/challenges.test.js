...
describe('updateChallenge', () => {
  it('should call database.child()', async () => {
    const challengeKey = 'mockChallengeKey';
    const body = { mockKey: 'mockVal' };

    await updateChallenge(challengeKey, body);

    expect(database.child).toBeCalledWith('challenges/mockChallengeKey');
  });
  ...
  it('should update the challenge', async () => {
    ...
    expect(database.child().update).toBeCalledWith(body, jasmine.any(Function));
  });
});
