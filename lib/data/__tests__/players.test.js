...
describe('setPlayerWithPriority', () => {
  it('should throw an error if no playerKey is specified', () => {
    try {
      setPlayerWithPriority(null);
    } catch (err) {
      expect(err.message).toBe('playerKey not provided!');
    }
  });

  it('should throw an error if no ref is specified', () => {
    try {
      setPlayerWithPriority('playerKey');
    } catch (err) {
      expect(err.message).toBe('ref not provided!');
    }
  });

  it('should throw an error if no body is specified', () => {
    try {
      setPlayerWithPriority('playerKey', 'ref');
    } catch (err) {
  ...
  it('should set the player', () => {
    setPlayerWithPriority('playerKey', 'ref', 'body');
