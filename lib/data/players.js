...
export const findPlayerByDisplayNameOnce = async displayName =>
  playersDb
    .orderByChild('normalizedDisplayName')
    .equalTo(displayName.toLowerCase())
    .once('value');

export const findPlayerByUidOnce = async uid =>
  playersDb.orderByChild('uid').equalTo(uid).once('value');
...
export const listPlayerChallengesByStatus = (
  playerKey,
  cb,
  status = 'active',
  listenStatus = 'on'
  ...
  // TODO: paginate this!
  if (cb) {
    return playersDbByCustom(`/${playerKey}/challenges/${status}`)[
      listenStatus
    ]('value', cb);
  }
  return playersDbByCustom(`/${playerKey}/challenges/${status}`)[listenStatus](
    'value'
  );
};

export const setPlayerWithPriority = (
  playerKey,
  ref,
  body,
  priority = 0 - Date.now()
